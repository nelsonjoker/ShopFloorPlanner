...
public class OperationIncrementalScoreCalculator {
    ...
    private Map<Operation, Long> mOperationDelays;
    ...
    private Score mScoreGrouping;	//maximizes grouping and provides nonvolatile replaning incentive 
    ...
    public void beforeVariableChanged(Object entity, String variableName) {
    	...
    	if(variableName.equals("startDate")){
    		retract((Operation) entity);
    	}else if(variableName.equals("resource")){
    		retract((Operation) entity);
    	}else if(variableName.equals("firstStart")){
    		retract((WorkOrder) entity);
    	}else if(variableName.equals("operationList")){
    ...
    private void insert(Operation op) {
        ...
        tracker.insert(op.getDuration(), op);
        //hardScore += tracker.getHardScore();
    ...
    private void retract(Operation op) {
    	...
    	ResourceCapacityTracker tracker = resourceCapacityTrackerMap.get(op.getResource());
        //hardScore -= tracker.getHardScore();
        tracker.retract(op.getDuration(), op);
        //hardScore += tracker.getHardScore();
    ...
    private void insert(WorkOrder wo) {
    	...
    	*/
        ...
        for (StockItemProductionTransaction stockProduction : wo.getProducedTransactionList()) {
	        ...
	        List<WorkOrder> invoiceList = invoiceEndDateMap.get(stockProduction.getVCR());
	        if(invoiceList == null){
	        	//invoiceList = new LinkedList<>();
	        	invoiceList = new SortedArrayList<WorkOrder>(new Comparator<WorkOrder>() {
					public int compare(WorkOrder o1, WorkOrder o2) {
						return o1.getLastEnd().compareTo(o2.getLastEnd());
					}
				});
	        	invoiceEndDateMap.put(stockProduction.getVCR(), invoiceList);
	        }
	        invoiceList.add(wo);
            ...
            /*
	        TreeSet<WorkOrder> invoiceList = invoiceEndDateMap.get(stockProduction.getVCR());
	        if(invoiceList == null){
    ...
    private void retract(WorkOrder wo) {
        ...
        */
        ...
        for (StockItemProductionTransaction stockProduction : wo.getProducedTransactionList()) {
            ...
            assert(invoiceEndDateMap.containsKey(stockProduction.getVCR()));
	        List<WorkOrder> invoiceList = invoiceEndDateMap.get(stockProduction.getVCR());
	        assert(invoiceList.contains(wo));
	        mScoreMakespan += invoiceList.get(invoiceList.size() - 1).getLastEnd() - invoiceList.get(0).getFirstStart();
    ...
    */
    ...
    public Score calculateScore() {
    	...
    	long workStockScore = 0;
    	for(IStockItemCapacityTracker tr : stockTrackerMap.values()){
    		workStockScore += tr.getSoftScore();
    	}
    	...
    	mScoreGrouping = Score.valueOf(0, 0);
    	...
    	for(Resource r : mResourceOperationMap.keySet()){
    		...
    		for(Operation o : sorted){
    			if(o.getNextOperation() != null){
    				long dly = o.getEndDate() - o.getNextOperation().getStartDate();
    				//long dly = o.getStartDate() + o.getUnitDuration() - o.getNextOperation().getStartDate();
    				if(dly > 0)
    					mScoreStock -= dly;
    			}
    		}
        ...
        return Score.valueOf(new long[] { mScoreDeliveryDateBehind, mScoreStock, mScoreResourceCapacity}, new long[] { mScoreGrouping.getHardScore(), mScoreGrouping.getSoftScore(), mOverlappedOperationScore, mScoreDeliveryDateAhead, mScoreMakespan, mIdleResourceScore});
