...
//    private List<WorkOrder> mDuplicateWorkOrderFilter;
    ...
    public ExecutionGroupClusterTracker(){
    	...
    	//mScore = 0;
    	//mVolatileScore = 0;
    	mScore = HardSoftScore.valueOf(0, 0); 
//    	mDuplicateWorkOrderFilter = new ArrayList<WorkOrder>();
    ...
    public void insert(long startTime, WorkOrder wo) {
    	...
    	if(k != null){
    		...
    		if(beforePrevEntry != null)
    			mScore = mScore.subtract(score(k.getValue(), beforePrevEntry.getValue()));
    		if(prevEntry != null)
    			mScore = mScore.subtract(score(k.getValue(), prevEntry.getValue()));
    		if(nextEntry != null)
    			mScore = mScore.subtract(score(nextEntry.getValue(), k.getValue()));
    		if(afterNextEntry != null)
    			mScore = mScore.subtract(score(afterNextEntry.getValue(), k.getValue()));
    		...
    		if(nextEntry != null && beforePrevEntry != null)
    			mScore = mScore.subtract(score(nextEntry.getValue(), beforePrevEntry.getValue()));
    		if(nextEntry != null && prevEntry != null)
    			mScore = mScore.subtract(score(nextEntry.getValue(), prevEntry.getValue()));
    		if(afterNextEntry != null && prevEntry != null)
    			mScore = mScore.subtract(score(afterNextEntry.getValue(), prevEntry.getValue()));
    		...
    		if(prevEntry != null && nextEntry != null)
    			mScore = mScore.add(score(nextEntry.getValue(), prevEntry.getValue()));
    		
    		//System.out.println("adding key "+ startTime);
    		
    	}
		if(beforePrevEntry != null)
			mScore = mScore.add(score(k.getValue(), beforePrevEntry.getValue()));
		if(prevEntry != null)
			mScore = mScore.add(score(k.getValue(), prevEntry.getValue()));
		if(nextEntry != null)
			mScore = mScore.add(score(nextEntry.getValue(), k.getValue()));
		if(afterNextEntry != null)
			mScore = mScore.add(score(afterNextEntry.getValue(), k.getValue()));
    ...
    public void retract(long startTime, WorkOrder wo) {
		...
		if(beforePrevEntry != null)
			mScore = mScore.subtract(score(ex, beforePrevEntry.getValue()));
		if(prevEntry != null)
			mScore = mScore.subtract(score(ex, prevEntry.getValue()));
		if(nextEntry != null)
			mScore = mScore.subtract(score(nextEntry.getValue(), ex));
		if(afterNextEntry != null)
			mScore = mScore.subtract(score(afterNextEntry.getValue(), ex));
		...
		if(ex.size() == 0){
			...
			if(nextEntry != null && beforePrevEntry != null)
				mScore = mScore.add(score(nextEntry.getValue(), beforePrevEntry.getValue()));
			if(afterNextEntry != null && prevEntry != null)
				mScore = mScore.add(score(afterNextEntry.getValue(), prevEntry.getValue()));
    ...
    private HardSoftScore score(List<WorkOrder> curr, List<WorkOrder> prev){
    	int hardScore = 0;
    	int softScore = 0;
    	
    	
    	List<ExecutionGroup> currGroups = new ArrayList<ExecutionGroup>(10);
    	for(WorkOrder wo : curr){
    		currGroups.addAll(wo.getExecutionGroups());
    	}
    	List<ExecutionGroup> prevGroups = new ArrayList<ExecutionGroup>(10);
